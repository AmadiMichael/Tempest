...
async function prove(witness) {
  const wasmPath = path.join(
    __dirname,
    "../build/shielded_transfer_js/shielded_transfer.wasm"
  );
  const zkeyPath = path.join(
    __dirname,
    "../build/shielded_transfer_circuit_final.zkey"
  );
...
async function getProve(
  ...
  const rootAfterChangeWasAdded = await tree.root();

  const topNodes2 = await tree.getTopTwoElements();
  const x2 = await tree.path(newLeafIndex + 1);
  await tree.insert(destCommitmentHash);
  const rootAfterDestWasAdded = await tree.root();
  ...
  const witness = {
    ...
    destNullifier: BigNumber.from(destNullifier).toBigInt(),
    ...
    topNodes2,
    afterPathElements2: x2.path_elements,
    afterPathIndices2: x2.path_index,
  };
